...
There are SQL, SQLite, SQL Server, MySQL, PostgreSQL, Oracle, "NoSQL" and others.
...
They are related through keys, we can see the relationship using the schema.
...
| SQL | What it does | dplyr |
|----|----|----|
| SELECT | Choose a column... | select, mutate, count, and more |
| FROM | ...the data frame |  |
| WHERE | A condition on a variable | filter() |
| GROUP BY | Subset the data for computation | group_by() |
| HAVING | A condition on grouped variables | filter() for groups |
| ORDER BY | Used to arrange the data (use DESC too) | arrange(), head(), slice_min() slice_max() |
| LIMIT | Shows only the first so many rows | head(), tail() |
| COUNT | Used with Select to count rows | count() |
| IS NULL | Used when searching for missing values | is.na() |
...
In SQL, with the wrong permissions you can really mess up your database. 

R is good at these things.

## CSI 242 ADM

At HCC we have a clas dedicated to creating and running a database. 

It is called Applied Database Management. 