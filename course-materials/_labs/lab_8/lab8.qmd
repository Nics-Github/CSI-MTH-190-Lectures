...
In this lab you are making several maps. The primary goal is to get you to recognize the difference between a population map, and a normalize map. You will also practice a lot of the wrangling skills that you've build up in this course. Check the rubric, you will be graded on each of your maps.
...
The data is saved both as a csv file in the data/ folder.

```{r}
# Edit and run this chunk to load the data without google sheets (if you are not on the server).
us_can_recovery_2022 = read_csv("_____")
mexico_recovery_pre_2023 = read_csv("data/mexico_data.csv")
```

## North American Map

I found an old map of North America that we can use for this lab. If you find another reasonably sized North American map with the states and provinces on it, please let me know.

```{r}
# I made a North America Map for you run the chunk to load it in.
...
# I added some layers to make things nice:
  theme_void()+
...
```{r}
##### Geocoding is annoying on the server. If you would rather build a tibble and manually write in the lat and long you can, we did this in the non-lab on mapping. If all is working correctly, this will take 7 seconds.
...
This is not an exciting map, I only see 7ish dots, not seven butterflies traveling south. I guess one butterfly rarely gets spotted again across state boarders. This shows the limits of citizen science. Not many people participate so the data is spotty.
...
# Note: The data frame "butterfly_location_data_state" is loaded into the environment 
load("data/butterfly_location_data_state.Rdata")

```

We would expect locations in the US with more people to have more sightings. So instead of plotting the overall sightings we'll make a Chloropleth map that colors the sightings by state as a percentage of the total sightings. We should always be mindful and ask ourselves if " we are really just creating a population map ? "
...
-   join the butterfly_location_summary to north_am, be sure to have north_am on the "left" so you don't lose simple features. Choose the correct join, we only care about state/provinces that have has butterfly sightings.
...
north_am <- north_am |>
  inner_join(butterfly_location_summary, by = c(____=_____)) 
...
north_am = north_am |> inner_join(_____) |>
  mutate(prop = ______)
...
These butterflies end up in Mexico for the winter. Using mexico_recovery_pre_2023 data
...
```{r}
# Read in the mexico csv 
mexico_locations = read_csv("____")
...
In Mexico volunteers are paid to collect and report tagged butterflies. Make a final map showing which states these butterflies end up in. You want your result to looks like the us_mex_can.png below.

![](us_mex_can.png)

This is a little challenging, but you can do it!

I did this with pull() and a few joins to make a vector of state/province names. Finally there was some count()-ing, joining, mutating with the coalesce() function and filtering before I drew my graph. You can also do it using other functions.

**Graded Map 4: Map of total counts US, CA, MEX**

```{r}
# I made you a vector of all the US CAN and MEX state abbreviations. You can use it for filtering or counting, if you want.
