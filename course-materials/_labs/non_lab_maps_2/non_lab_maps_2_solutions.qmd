...
No need to run the chunk below.
...
write_csv(vt_ski_resorts,"vt_ski_resorts.csv")
```
...
```{r}
vt_ski_resorts <- read_csv("vt_ski_resorts.csv") |> 
  st_as_sf(coords = c("long", "lat"), crs = 4326, remove = FALSE)
...
In the following example we'll plot out the population of each county on Mass using data from the america community survey.
...
mass_pop_orig <- 
  tidycensus::get_acs(
    geography = "county", 
    variables = "B01003_001", # Variable for population
    state = "MA",
    geometry = TRUE # Include shape files in downloaded data frame 
  ) |> 
  # Add centroids to each region using purrr package
  mutate(
...
## More variables for acs5

You can find all the variables here, but it is often easier to google them.

```{r}
# default year 2023
# acs is american community survey
# https://www.census.gov/programs-surveys/acs.html
variables <- tidycensus::load_variables(2023,"acs5") 
...
mass_pop_orig <- 
  tidycensus::get_acs(
    geography = "county", 
    variables = c("B03001_005"), # Variables for male/female sex under 5
    state = "MA",
    geometry = TRUE
  ) |> 
  # Add centroids to each region using purrr package
...
ggplot() +
  geom_sf(data = mass_pop, aes(fill = estimate)) +
  #geom_text(data = mass_pop, aes(x = lng, y = lat, label = County)) +
  labs(fill = "Number of Puerto Ricans\nEstimate")
