...
Start with n/total_births = prop and solve for total_births.
...
```{r}
babynames = babynames|>
    mutate(total_births = #### put code here)
view(babynames)
```
...
The slice family will subset rows based on conditions. `slice_max()` grabs the rows with maximum value. `slice_min()` the rows with the minimum value, there are also other slice\_\*() functions. Let's use `slice_max()` to find the five years when the most babies were born.
...
NOTE: When filtering by multiple values its important to use the %in% operator. When just one value you can use == instead. Also the c() structure in R is called a vector. It is similar to a list or an array in other languages. Data Frames are really just a bunch of vectors put together.
...
**Problem 4:** Use `case_when()` and `mutate()` to add a new column called "generation_name" that takes the year column and labels it with the correct generation. Save this dataframe as `generations`. I'll get you started.

**Solution:**

```{r}
generations = babynames |>
  mutate(generation_name = case_when(
    year >= 1883 & year <= 1900 ~ "Lost Generation",
```

**Problem 5:** Using the `generations` dataframe, group_by() and summarize() find the total number of births by generation, then use `geom_col()` to make a nice bar graph showing the generation_name on the y-axis and the total births on the x-axis. (Hint: it will be helpful to use the provided theme:
...
**Problem 8:** Use `filter()` and `str_detect()` to find the top three names from the Millennial and Zoomer generation that start with "E". You should only have to add **max** one line to your answer to number 7. Save your answer as `gen_top6` for problem 9, make sure the six names and their totals still show in the rendered document. I'll get you started:
...
generations |>
  ...
  filter( generation_name %in% c("Millennials", "Zoomers"))
```

**Problem 9:** For each generation in the `generations` dataset find the average number of the top 5 names you found in problem 8. Your answer should have 45 rows.
