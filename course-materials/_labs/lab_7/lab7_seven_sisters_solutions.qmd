...
Question: Consider the Check out the 2022 race results at this [website](https://aratrace.com/7-sisters-trail-race-4/). What sensitive data is presented in the website?

**Answer :** Names, city, state

## RVest

Rvest is a package for web scraping it was introduced in the scraping lecture on canvas.
...
seven_sisters_2022 <- "https://aratrace.com/7-sisters-trail-race-4/"

# pipe the url into read_html(). 

results_2022 <- seven_sisters_2022 |>
  read_html() |> 
  html_element("#table_1") |> 
  html_table()
```
...
names(results_2022) <-  str_replace_all(
    names(results_2022),
    pattern = " ",
    replacement = "_"         
                                    )
...
The tidyverse style guide tells us to make variables lowercase. So we should do that. Find the function in the cheatsheet that will make the column names lowercase and add it below.

```{r lower-case}
names(results_2022) <-  tolower(names(results_2022)
                                    )
names(results_2022)
```
...
```{r rename}
results_2019 <- results_2019 |>
  rename( net_elapsed = chip_elapsed)
```
...
**Answer:** 92 runners ran both races
...
**Answer:** CT, MA, NH, NJ, NY, PA, RI, VT
...
# results negative
results_joined |>
mutate (diff_in_time = net_elapsed2022 - net_elapsed2019)|>
summarize(mean(diff_in_time) )

# results positive
results_joined |>
mutate (diff_in_time = net_elapsed2019 - net_elapsed2022)|>
summarize(mean(diff_in_time) )

```

**Answer:** They improved from from 2019 to 2022, but not by much. Unfortunately we don't cover enoguh stats in this class to say wether this value is significant or not. 
...
```{r}
# Do your joins here. 
results_2017_2019 = results_2019 |> 
inner_join(results_2018, by= c("first_name","last_name"))|> 
inner_join(results_2017, by= c("first_name","last_name")) |>
select(first_name,last_name)
```

```{r}
results_2017_2019 |>
inner_join(results_2022, by= c("first_name","last_name"))|>
select(first_name,last_name)
```


Question: Based on the runners from the 2017-2019 race which runners do you guess would be present for the 2022 race? Which of those runners actually were present in 2022? 

**Answer:** I predict there will be 43 runners listed in the data frame above at the 2022 race. Of those 43, 17 runners returned in 2022. 


Figuring out the diehard runners of a race is not necessarily nefarious. You might be the race coordinator and want to reach out to these people for donations, that seems fine. Alternatively you might be a reporter writing for the local news. Would the original purpose of the data effect what you write about the race? What is the original purpose of the data? Do we want to get consent before we publish first and last names, do we need to publish the city and state where the runners are from? Could this information be used to stalk a runner?

**Answer:** Answers may vary. The original purpose of the data is for the runners and the race coordinators to track times. We definitely want consent before publishing names in an article. We probably would be okay with publishing states, maybe so on cities but checking with the runner before publishing an article that included their names and locations would be polite. 

Bonus. Make your document a little cooler by inserting the logo from the 7 sisters race here. Use Insert \> image from the visual mode. The come back to source mode to see how this gets coded.
