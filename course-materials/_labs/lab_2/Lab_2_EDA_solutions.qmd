...
subtitle: "Solutions Lab 2: Intro to Data"
...
You will do an exploratory data analysis on the `MLB_teams` survey data. You must answer all questions. If you have questions please post them to the discussion for this lab, or ask in class. 
...
2. What is the `lgID` for the Boston Red Sox. What does `lgID` mean? (You may have to search the internet, or ask a peer).

**Answer:**  AL is the value for the Red Sox. lgID is the league the team is in. American or National. 

3. Considering the help menu. What are the sources of this data?

**Answer:** The Lahman package and Wikipedia

4. For this problem, use the `count()` function from the dplyr package. How many teams are present in this data? (Hint: You may want to google examples of count() or see the documentation with `?count()`). 
...
**Answer:** I get 31 teams each having about 7 occurances. 
...
**Answer:** The Marlin's changed their name and thus their MLB team ID. Check the teamID: MIA, FLO. in the code chunk below I put those two together and get 30 teams. 
...
Take the code above and add another variable called `mean_loss` inside of the summarize function.  You should have two variables in your output: `mean_win` and `mean_loss`. You may want to google another example or check out the documentation for `summarize()`. 
...
# The code for the summary data goes here
MLB_teams |>
    summarize(mean_win = mean(W), mean_loss = mean(L))
```
...
6. We can summarize by statistics other than the mean(). Edit the code above and find the median() of the wins for the year. In what year had the most median wins?

```{r }
# The code for the summary data goes here
MLB_teams |>
    group_by(yearID) |>
    summarize(mean_win = mean(W), median_win = median(W))
```

**Answer:** 2009, there were more median wins. 
...
```{r }
# The code for the summary data goes here
mlb_attendance = MLB_teams |>
    group_by(lgID,yearID) |>
    summarize(total_attendance = sum(attendance))
```

8. Pipe mlb_attendance into ggplot() and use geom_col() to make a bar graph showing the summary you calculated in number 7. (Hint: geom_col() takes two variables x and y in the aes()). Your graph will be pretty boring, but we'll spice it up in problem 11. 
...
mlb_attendance |>
    ggplot()+
    geom_col(aes(x=yearID,y=total_attendance))
```

We can add lables by adding labs() after our geom. See my code for an example of this:
...
9. Add labels and a title to your graph from problem 8. 
...
MLB_teams |>
    group_by(lgID)|>
    summarise(avg_pop = mean(metroPop))|>
    ggplot()+
    geom_col(aes(x = lgID, y= avg_pop))

```

**Summarize your graphic with a sentence or two:** Answers will vary. But on average the national league has a slightly higher average population. 

## Revisit mlb_attendance

11. In chapter 2.1 from the reading this week. You saw examples of color coding graphs. We'll do this below. I'm interested in comparing the attendance between the two leagues by year. Grab your code from your column graph in problem 9 above and fill = lgID into the aes() function. 

```{r}
mlb_attendance |>
    ggplot()+
    geom_col(aes(x=yearID,y=total_attendance, fill = lgID))+
    labs(
        x = "year",
        y = "total attendance",
        title = "Attendance for the\nAmerican and National Leagues",
        caption = "MDSR r package, Lahman, v. 0.2.8"
    )
```
