...
subtitle: "Lab 3: Intro to Graphics"
author: "Schwab"
date: "today"
format:
  html:
...
Note: This lab was originally written by my colleagues at Smith College. I've edited it slightly. 
...
This lab will teach about some common topics we find when working with data:

- overplotting
- alpha, size and shape aesthetics
- color
- scale

The goal is to get you familiar with these topics. 

Something to keep in mind when reviewing this data:
...
```{r}
# If you cannot read in the file set the current working directory to the same folder your lab is in. Use the gear icon and choose setwd().
# Ignore the rest of this code for now.
library(tidyverse)
...
We'll look at the `ne_country_health_data_2021` This is the county health data from just the 6 new england state. 

```{r}
# Write your code below
head(ne_county_health_2021)
...
Copy and paste the code I created above into the chunk below. In the `geom_point()` function, set the *alpha* aesthetic to 0.75 and the *size* to 0.5. Notice the adjustments to the plot as you update these attributes. 

```{r}
# Paste the code from above and adjust it
ggplot(data = county_health_2021, 
       aes(x = Children_in_poverty_raw_value, 
           y = Math_scores_raw_value, 
           color = Broadband_access_raw_value),
...
**Answer**: One possible answer: As poverty rates increase math scores decrease.
...
```{r}
# Take the code from above and adjust the alpha aesthetic. 
ggplot(data = ne_county_health_2021, 
       aes(x = State_Abbreviation, 
           y = Flu_vaccinations_raw_value)) +
...
**Answer**: One Possible Answer: VT and ME have large disparity rates.
...
![](images/Lab3_plot1.png)

```{r}
# Write your code below
ne_county_health_2021 |>
    ggplot(aes(x=Poor_physical_health_days_raw_value, y = Life_expectancy_raw_value))+
    geom_point()+
    labs(
        x = "Reported Physically Unhealthy Days", 
        y = "Life Expectancy", 
        title = "Health Measure By County, New England, 2021"
    )
```
...
As we discussed (or will discuss), there are three types of color palettes that can be added to a plot:
...
ne_county_health_2021 |>
    ggplot(aes(
        x=Poor_physical_health_days_raw_value, 
...
**Answer**: One Possible Answer: I think CT has higher life expectancy and fewer unreported physically unhealthy days.
...
**Answer**: One Possible Answer: It looks like weathier people have higher life expectancy
...
# Paste the code below and adjust it
ggplot(data = county_health_2021, 
       aes(x = Population_raw_value,   #This should change to population
           y = HIV_prevalence_raw_value)) +
  geom_point(alpha = 0.75, 
             size = 0.5, 
             position = "jitter") +
  scale_y_binned(breaks = c(1000, 3000), 
                 labels = c("HIV Less Prevalent", "HIV More Prevalent")) +
  scale_x_binned(breaks = c(1000000,2000000,3000000,4000000), 
                 labels = c('1','2','3','4')) +
  labs(title = "Health Measures in the US, 2021",
       x = "Population binned by millions", 
       y = "HIV Prevalence Measure") +
  theme_minimal()
```

No answer requested by it seems like smaller counties have larger HIV prevelance than larger ones.

Remember to Render this lab before you put it on canvas.  